...
export class Rating {
  ...
  public getRatingForPoint(point: ForecastPoint): number {
    const swellDirectionPosition = this.getPositionFromLocation(
      point.swellDirection
    );

    const windDirectionPosition = this.getPositionFromLocation(
      point.windDirection
    );
  ...
  public getRatingBasedOnWindAndWavePositions(
    ...
    if (wavePosition === windPosition) {
      return 1;
    } else if (this.isOffshoreWind(wavePosition, windPosition)) {
      return 5;
    }
